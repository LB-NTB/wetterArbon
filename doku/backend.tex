...
\noindent
\Diskussionspunkt{Warum kann normiertes Format in der API nicht angewendet werden?} \newline
\Diskussionspunkt{The delimiter between the date part and the time part can be a T or any sequence of space characters} \newline
\Diskussionspunkt{\url{https://mariadb.com/kb/en/library/date-and-time-literals/}} \newline
...
% SQL-Injection ist ein grossen Problem
...
% Wie im Kapitel Daten der neuen Wetterstation mit Tableau \ref{kap:Tableau} erwähnt, müssen die Daten für das Tableau manuell übertragen werden. Dies bedeutet, dass für die historische Seite kein Datenbankzugriff notwendig ist.
...
Da beim Server der Speicherplatz kein ausschlaggebender Punkt ist, wurde die zweite Variante gewählt.
